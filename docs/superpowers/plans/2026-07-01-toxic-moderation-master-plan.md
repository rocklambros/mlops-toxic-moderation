...
3. `infra/aws/bootstrap.sh`: idempotent, eleven ordered steps per spec section 6, which names every verified CLI operation. Step 7 is an operator gate that pauses for root-mail confirmation before step 8 deletes root credentials. Note the root-credential gotchas in spec section 5.2: `sts assume-root` needs a regional endpoint, caps at 900 seconds, requires `sts:AssumeRoot` on the caller, and returns `NoSuchEntity` on a fresh account with no root credentials, which the script must treat as success.
