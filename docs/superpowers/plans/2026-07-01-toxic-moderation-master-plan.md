...
**Prerequisites (verify before task 1):** AWS CLI v2 installed (v1.35.0 is currently installed and is not sufficient), Terraform 1.11 or newer (1.5.7 is currently installed, and S3 native state locking went GA in 1.11), `gh` authenticated, and a delivered test message to `rock+aws-mlops-toxic@rockcyber.com` (Mimecast recipient validation is the risk).
...
3. `infra/aws/bootstrap.sh`: idempotent, ten ordered steps per spec section 6, which names every verified CLI operation. Note the root-credential gotchas in spec section 5.2: `sts assume-root` needs a regional endpoint, caps at 900 seconds, requires `sts:AssumeRoot` on the caller, and returns `NoSuchEntity` on a fresh account with no root credentials, which the script must treat as success.
