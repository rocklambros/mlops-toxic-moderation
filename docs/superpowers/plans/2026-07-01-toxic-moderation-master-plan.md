...
**Prerequisites (verify before task 1):** AWS CLI v2 installed (v1.35.0 is currently installed and is not sufficient), Terraform 1.10 or newer (1.5.7 is currently installed and lacks S3 native state locking), `gh` authenticated, and a delivered test message to `rock+aws-mlops-toxic@rockcyber.com` (Mimecast recipient validation is the risk).
...
2. `infra/aws/scp-sandbox-guardrails.json`: region lock, deny `iam:CreateUser` and `iam:CreateAccessKey`, Graviton instance-type allowlist, deny GPU and metal families, cap RDS at `db.t4g.small`, deny leaving the org, deny CloudTrail and GuardDuty tampering. Validate the JSON before attaching.
3. `infra/aws/bootstrap.sh`: idempotent, ten ordered steps per spec section 6. Enable the SCP policy type, create the `Sandbox` OU, create and attach the SCP, create the member account and poll to `SUCCEEDED`, move it into the OU, delete its root credentials, create and assign the two Identity Center permission sets, assume `OrganizationAccountAccessRole` to set the alias and create the Terraform state bucket, write the gitignored outputs file. **Verify the centralized root access management API surface against current AWS documentation before implementing step 7.**
